...
function displayMarkers(places) {
	...
	for(var i=0; i<places.length; i++) {
		...
		temp_map.set("title", places[i].aptName);
		temp_map.set("place", places[i]);
		marker_infos.push(temp_map);
...
// 검색 결과 창에 mouseenter했을 때 호출되는 함수입니다.
// displayInfowindow를 실행합니다.
function displayInfowindowByResult(index) {
	if (customOverlay)
		customOverlay.setMap(null);
	var marker = markers[index];
	var title = marker_infos[index].get("title");
	var place = marker_infos[index].get("place");
...
function displayInfowindow(marker, title, place) {
	var content = `
		<div class="overlaybox">
			<span class="apt_code" style="display:none">${place.aptCode}</span>
			<span class="boxtitle">${title}</span>
			<span class="closebox" onclick="closeOverlay()">❌  </span>
